...
class AlertEngine:
    ...
    def _insert_alert(self, alert: dict):
        # Print the alert nicely in the console no matter what
        log.warning(f"🚨 Alert System Triggered -> {alert['message']}")
        
        # If dry-run mode is active, skip the PostgreSQL insert attempt completely!
        if self.dry_run:
            log.info(f"[DRY RUN] Successfully bypassed database insertion for this alert.")
            return
